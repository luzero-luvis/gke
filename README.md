...
See the [architecture and guidance mapping](docs/architecture.md) for assumptions, costs and organization-level decisions. For `gcloud`/Terraform auth setup and a least-privilege IAM role list, see [GCP CLI & Terraform authentication setup](docs/gcp-auth-setup.md). For what's actually deployed in the running test environment, how it was built, and the current live architecture diagram, see the [deployment log](docs/deployment-log.md). For the separate `mts-ijp` app stack sharing this same cluster/Gateway, see the [mts-ijp deployment log](docs/mts-ijp-deployment.md).
